document.addEventListener('DOMContentLoaded', async () => {
    ...
    function getPayload() {
        ...
        return {
            jobTitle: fd.get('jobTitle'), company: fd.get('company'), jobDescription: fd.get('jobDescription'),
            constraints: { maxPages: parseInt(fd.get('maxPages')), fontSize: parseInt(fd.get('fontSize')) },
    ...
    document.getElementById('generate-btn').addEventListener('click', async (_e) => {
        ...
        const res = await fetch('/api/snapshots', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ applicationId: appId, constraints: payload.constraints, userScoredInventory: payload.inventory })
        });
    ...
    document.getElementById('score-btn').addEventListener('click', async (_e) => {
        ...
        currentInventory = await (await fetch('/api/score', {
            method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ jobDescription: jobDescription })
        })).json();
